...
export default function WeatherDetails({ onBack }: WeatherDetailsProps) {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      let latitude = 25.3176; // Default to Varanasi, India coordinates
      let longitude = 82.9739;
      let defaultLocationName = "Varanasi, UP";
      ...
      try {
        ...
        console.warn("⚠️ Geolocation denied or failed, using default location (Varanasi).");
      }
      ...
      try {
        ...
        // Safe fetch for forecast (handles cases where onecall is unauthorized)
        let forecastData: any = { daily: [] };
        try {
          ...
          if (forecastRes.ok) {
            ...
            // Process the 5-day / 3-hour forecast into daily format
            const dailyMap = new Map();
            data.list.forEach((item: any) => {
              const dateStr = new Date(item.dt * 1000).toLocaleDateString([], { month: "short", day: "numeric" });
              if (!dailyMap.has(dateStr)) {
                dailyMap.set(dateStr, item);
              }
            });
            forecastData.daily = Array.from(dailyMap.values()).map((day: any) => ({
              dt: day.dt,
          ...
          console.error("Failed to load forecast data:", forecastErr);
        }
        ...
        setCurrentWeather({
          ...
          uvIndex: 5, // Default/Fallback UV index
          ...
          location: currentData.name || defaultLocationName || "Unknown",
        });

        // Map forecast data
        const days = ["Today", "Tomorrow", "Wed", "Thu", "Fri", "Sat", "Sun"];
        const mappedForecast: ForecastDay[] = forecastData.daily.slice(0, 5).map((day: any, index: number) => ({
          day: days[index] || new Date(day.dt * 1000).toLocaleDateString([], { weekday: "short" }),
        ...
        // Minimum mock setup so it doesn't stay blank
        setCurrentWeather({
          temp: 32,
          condition: "Sunny",
          feelsLike: 35,
          ...
          sunrise: "05:30 AM",
          sunset: "06:45 PM",
          location: defaultLocationName || "Unknown",
        });
      }
    };
